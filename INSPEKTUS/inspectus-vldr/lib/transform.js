...
// standalone CLASS "Damage" must NEVER be written in the remarks cell, BUT the damage must still be
// LISTED. Every damage contributes its PART-TYPE code + the inspector's free-text COMMENT; only the
// literal classification word "Damage" is dropped. A "Damage" with no comment therefore shows just
// its code (never blank, never the word). "No Damage Evidence" and "Observation" are written unchanged.
