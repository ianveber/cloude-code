-- INSPECTUS Center — initial schema migration (Phase 2: auth + persistence)
-- CANONICAL source of truth. Applied via `supabase db push` (or paste into SQL Editor).
-- Idempotent (if not exists / create or replace / drop policy if exists) — safe to re-run.
-- Single shared INSPECTUS workspace (single-tenant). Signup gated to @inspectus.si + an admin allowlist.
-- ../schema.sql is a kept-in-sync convenience copy for zero-tooling dashboard paste.
