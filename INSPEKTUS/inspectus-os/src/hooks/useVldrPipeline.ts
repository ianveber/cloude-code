...
export function useVldrPipeline() {
  ...
  const [complete, setComplete] = useState(false);   // AI enrichment settled → safe to persist
  ...
  const handleFile = useCallback(async (file: File) => {
    setError(""); setReady(false); setComplete(false); setValidation(null); setSummary(null);
    try {
      ...
      // AI enrichment runs after the instant deliverables. The save (SaveStatus) waits
      // for `complete`, so the stored run keeps its summary + validation — previously the
      // save fired on `ready` and persisted both as null.
      try {
        setStep("validate", "active");
        const dmgPayload = v.flatMap((veh: any, vi: number) =>
        ...
        setValidation(await claude.validate(dmgPayload)); setStep("validate", "done");

        setStep("summary", "active");
        setSummary(await claude.summarize(computeStats(v))); setStep("summary", "done");
      } catch {
        // AI enrichment failed — instant deliverables stay valid; don't block the save.
        setStep("validate", ""); setStep("summary", "");
      } finally {
        setComplete(true); // persist now, with whatever AI enrichment we obtained
      }
    } catch (e: any) {
  ...
  return { rawRows, setRawRows, vehicles, maxDamages, validation, summary, steps, error, ready, complete, handleFile, regroup };
}
